...
import (
	...
	"github.com/foxmeller/go_final_project/constants"
	"github.com/foxmeller/go_final_project/helpers"
)
...
func (h *Handler) GetDateTask(w http.ResponseWriter, r *http.Request) {
	...
	if nowstr == "" {
		...
		_, _ = w.Write([]byte(`{"error": "now date missing"}`))
		return
	}
	...
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte(`{"error": "invalid now date format"}`))
		return
	}
	...
	if date == "" {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte(`{"error": "date missing"}`))
		return
	}
	...
	if repeat == "" {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte(`{"error": "repeat missing"}`))
		return
	}
